...
funding: ""
params:
  project_title: My Little Project
...
# Assign score values
alignment_score <- round(as.numeric(0.02), 2)
dynamics_score <- round(as.numeric(0.991304347826087), 2)
cascade_score <- round(as.numeric(0.839572192513369), 2)

# Load Dataframes
alignment_df <- read_csv("data/alignment_data.csv")
cascade_df <- read_csv("data/cascade_data.csv")
dynamics_df <- read_csv("data/dynamics_data.csv")
indicators_df <- read_csv("data/indicators_data.csv")
...
My Little Project is a good project.
...
knitr::include_graphics(
  "plots/dashboard_plot.png"
 )

```

::: {.callout-note}
CEnTR\*IMPACT provides quantitative metrics specifically designed to effectively
communicate the value of community-engaged scholarship within academic evaluation
systems. These metrics enable scholars to demonstrate impact to review committees,
research teams, and funders through compelling data-driven narratives that complement
...
Direct Indicators are status counts of information including the number of partner
institutions; the amount of time spent working together; the total number of attendees
and participants at events and meetings; the total number of tools, documents, processes
created to ensure fairness and accountability through the partnership; the total
number of products---e.g., community-facing, scholar-facing---created; if engaged
learning is a component of the project; and the number of agreed-to outcomes that
...
`r knitr::include_graphics("plots/alignment_plot.png")`
...
The Project Dynamics Score ($S_d$) is `r dynamics_score`.
...
The Cascade Effects Score ($S_c$) is `r cascade_score`.
...
knitr::include_graphics("plots/cascade_plot.png")
```
...
![](centr-impact-logo.png){fig-align="center" width=10%}
